...
| E2E Velero (kind) | a preflight plus the twelve V1-V10 scenarios, thirteen tests in all. A deliberate release gate ([ADR 0019](docs/adr/0019-integration-and-e2e-tests-are-the-product.md)) |
| E2E rclone (minio) | R1-R7 against the demo stack, over both proxy endpoints. A deliberate release gate ([ADR 0019](docs/adr/0019-integration-and-e2e-tests-are-the-product.md)) |
| E2E s3cmd (minio) | S1-S7, the same. Its own job, never bundled with rclone's: a red gate has to name the client, and one client's flake must not withhold the other's verdict |
...
request waits on a check that never reports. `E2E rclone (minio)` and
`E2E s3cmd (minio)` are the fourteenth and fifteenth, added 2026-09-13.
...
| | |
|---|---|
| From `test-pipeline.yml` | Malware Scan (Source Code), Unit Tests, Race Detector, GoSec Security Scan, Vulnerability Check, Code Linting, Helm Chart, Integration Tests, Combined Coverage, Conformance (minio), Conformance (localstack), E2E Velero (kind), E2E rclone (minio), E2E s3cmd (minio) |
