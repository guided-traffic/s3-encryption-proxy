...
func TestUtlHandleS3Error_WriteFailureIsLogged(t *testing.T) {
	logger, buf := UtlCaptureLogger(logrus.DebugLevel)
	w := UtlNewFailingWriter(errors.New("connection reset by peer"))

	HandleS3Error(w, logger, &types.NoSuchKey{}, "Failed to get object", "test-bucket", "test-key")
...
func TestUtlHandleS3Error_ResourceComposition(t *testing.T) {
	cases := []struct {
		name         string
		bucket       string
		key          string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			HandleS3Error(w, logger, &types.NoSuchKey{}, "Failed to get object", tc.bucket, tc.key)
			...
			assert.Equal(t, "Failed to get object", entry.Message)
...
func TestUtlHandleS3Error_EscapesResource(t *testing.T) {
	...
	HandleS3Error(w, logger, &types.NoSuchKey{}, "Failed to get object", bucket, key)
...
func TestUtlHandleS3Error_DoesNotLeakBackendDetail(t *testing.T) {
	...
	HandleS3Error(w, logger, err, "Failed to get object", "test-bucket", "test-key")
...
func TestUtlHandleS3Error_StatusDrivesLogLevel(t *testing.T) {
	cases := []struct {
		name       string
		err        error
		wantStatus int
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			HandleS3Error(w, logger, tc.err, "op failed", "b", "k")
...
func TestUtlHandleS3Error_InternalTextStaysInternal(t *testing.T) {
	const secret = "/etc/s3ep/private-key.pem"
	logger, buf := UtlCaptureLogger(logrus.DebugLevel)
	w := httptest.NewRecorder()

	HandleS3Error(w, logger, fmt.Errorf("load KEK from %s: permission denied", secret), "op failed", "b", "k")
...
// "error detail" line.
func TestUtlHandleS3Error_NilError(t *testing.T) {
	logger, buf := UtlCaptureLogger(logrus.DebugLevel)
	w := httptest.NewRecorder()

	HandleS3Error(w, logger, nil, "no error supplied", "b", "")
