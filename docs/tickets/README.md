...
**State (2026-09-13, `feat/major-v5`).** Eighteen live ticket files and three in
[archive/](archive/) — 011, 013 and 024 were archived on 2026-09-13, the first
under the amended [ADR 0022](../adr/0022-tickets-are-work-lists-that-get-archived.md),
which closes a ticket by moving it rather than deleting it. Plus
...
three tickets under the rule as it stood then (020, the development-license one,
on 2026-09-09; 022, the S3-surface one, with wave 2; and 014, the
upload-checksum one, with wave 3), and those three are in the git history alone.
**015 and 016 are finished and stay here for now**: 015 ties its own close to the
5.0.0 merge, and 016 still has chart decisions with no home outside the ticket —
extraction is the close, so it cannot move until they have one. 010's verdict —
archive it, or state what is still wanted — is still open in the work list of
[023](023-major-v5.md), and its measurement tables belong in ADR 0020 first. [017](017-filename-encryption.md) and
...
| [015](015-configuration-hygiene.md) | **Done 2026-09-11, archived with the merge** | The dead knobs went with the deletion round (N-5) and every control it owed landed: `max_clock_skew_seconds` on both authentication forms, the pre-signed lifetime ceiling, the refusal to start on a plain-HTTP backend, the metadata-prefix shape, and an unknown configuration key refusing the start | P-11 |
| [016](016-helm-chart-fixes.md) | **Done 2026-09-11, awaiting archiving** | Chart: `checksum/config` rollout, TLS-aware probes, the two values files that failed `helm template`, and the CI that would have caught them — all twenty-one items done, plus the chart's own TLS listener ([ADR 0026](../adr/0026-the-proxy-terminates-tls-at-its-own-service.md)), which the ticket had put out of scope | P-10 |
...
| [034](034-etag-form-and-the-last-chance-sweep.md) | Open, **created 2026-09-13**, item 2 in 5.0.0; **item 1 decided and built the same day** (ADR 0032: the entity tag is a change token and carries a `-0` marker), leaving the routing gap and the six major-only tightenings | The last-chance sweep: what only a major can change. The entity tag is the backend's ciphertext MD5 in the shape of a content digest — an ad-hoc rclone probe failed every single-part upload against it — and the candidate answer is a `-0` marker under an encrypting provider, decided only after an rclone and an s3cmd end-to-end suite exist and have run (`test/e2e/rclone/`, `test/e2e/s3cmd/`); `UploadPart` under the exit provider reads the whole part without a bound and is forwarded while it arrives instead; six tightenings that ADR 0013 D11 and ADR 0018 D5 make major-only, each with a recommendation, decided in discussion | — |
| [035](035-the-pinned-defect-sweep.md) | Open, **created 2026-09-13**, the unit half built the same day | The pinned-defect sweep: 27 tests across the tree asserted the answer the product gives today where that answer is known to be wrong. They now assert the target. The unit half is built and `make test-unit` is green; the integration suites still do not pass on this branch. 55 candidates were checked adversarially against the ADRs and 28 were refused as decided behaviour. The entity tag (ADR 0010 D12), the absent `x-amz-request-id` (ADR 0008 D12), `DeleteObjects` input validation, four routing defects, two bucket sub-resources, the multipart part order, the response overrides (ADR 0007 D1/D2), `?torrent`, one missing startup check and two messages that promise an implementation | — |
