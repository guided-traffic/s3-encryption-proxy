...
write-side prefix refusal, `ListParts`, two configuration remainders, the
performance after-column and the documentation.

One gate is outstanding for the whole branch, not for one ticket: the Velero e2e
suite has not been run since the format landed.
...
| P-7 | `ListParts` returns a fabricated empty `<ListPartsResult>` at 200, so a client verifying an upload is told it has zero parts; `ListMultipartUploads` is 501. With P-8, orphaned uploads existed and could not be enumerated. Now marshalled from a struct (F-13) but still reporting zero parts — the `TODO` is at [list.go:65](../../internal/proxy/handlers/multipart/list.go#L65) | Open, item 10 of [013](013-storage-format-v2.md) |
