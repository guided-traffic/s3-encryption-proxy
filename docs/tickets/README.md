...
([028](archive/028-upload-and-read-performance-round.md)); between them they deleted
...
| [038](038-s3-encryption-operator.md) | Announced 2026-09-14, **not scheduled** | A Kubernetes operator provisioning proxy instances through Custom Resources: backends, licence distribution, credential Secrets. It ships its own chart; `s3-encryption-proxy` stays the single-instance one ([ADR 0033](../adr/0033-a-proxy-instance-holds-its-uploads.md)). Configuration is read at start only, so a changed backend rolls the pod | — |
| [035](035-the-pinned-defect-sweep.md) | Open, **every suite green 2026-09-13**, two process items left | The pinned-defect sweep: 27 tests across the tree asserted the answer the product gives today where that answer is known to be wrong. They now assert the target. The unit half was built the same day, and every suite is green on this branch — unit, integration, integration over TLS, conformance and all three end-to-end suites. 55 candidates were checked adversarially against the ADRs and 28 were refused as decided behaviour. The entity tag (ADR 0010 D12), the absent `x-amz-request-id` (ADR 0008 D12), `DeleteObjects` input validation, four routing defects, two bucket sub-resources, the multipart part order, the response overrides (ADR 0007 D1/D2), `?torrent`, one missing startup check and two messages that promise an implementation | — |
