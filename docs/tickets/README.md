...
| [036](036-select-is-refused-with-the-wrong-error.md) | Open, **found 2026-09-13** | `SelectObjectContent` answers `403 SignatureDoesNotMatch` where ADR 0007 D8 decides `501 NotImplemented`: the request is refused by authentication before it reaches the handler that refuses the verb. MinIO verifies the same SDK signature, so it is this proxy's defect and not the client's. Found by giving a test that swallowed its error an assertion | — |
