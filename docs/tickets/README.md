...
**State (2026-09-18).** Twelve live ticket files; everything
...
[037](037-multiple-backends.md) is an announcement so that 5.0.0's configuration
does not foreclose it; nothing is built. **[036](036-high-availability.md) and
[038](038-s3-encryption-operator.md) are no longer among them.** 038 was refined
on 2026-09-16 against the security decisions of a sibling operator: its scope is
decided — one cluster-scoped operator over namespaced Custom Resources, no
cross-namespace references in a resource, backend credentials read and the
proxy's own client credential minted, cert-manager adopted into the kind
end-to-end setup — and a second round the same day settled the operator's
Secret privileges, the resource's schema and where a provisioned instance's
licence comes from. It owes two ADRs for all of it. 036 was refined on
2026-09-15 and 2026-09-16, in three rounds, and its shape is decided — a shared
session table in an externally provided Valkey with Sentinel — which pulled the
multipart idle clock off the not-scheduled list, because its throttled store
looked like the lease heartbeat that design needs, and produced
...
list, refined on 2026-09-18 into a `ca_file` feature plus documentation. [040](040-managed-buckets.md) is a plan, raised
