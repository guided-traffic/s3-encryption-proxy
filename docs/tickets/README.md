...
| [032](032-open-decisions-from-the-documentation-audit.md) | Open, **decisions only (2026-09-12)** | The thirteen findings of the documentation audit that are not documentation defects: a re-sent multipart part that produces an object which never reads, `CompleteMultipartUpload` echoing the backend's encryption headers, the chart's NetworkPolicy default blocking the ports the chart opens, `license_file` falling back to six other paths, and nine smaller ones. All thirteen decided with the owner on 2026-09-12 and 2026-09-13; none of the work is built. Four carry an ADR obligation | — |
| [033](033-out-of-band-recovery-path.md) | Open, **not scheduled (2026-09-12)** | A deliberate out-of-band tool that decrypts an object whose metadata or stored bytes are damaged, under the owner's explicit risk. The proxy must never do this — it may not serve an unauthenticated byte. Raised out of 032 D7; the requirement and what the format already gives it are recorded, the design is not started | — |
