...
`docs/security/`), then `git grep` its number and clear whatever is
...
| [038](038-s3-encryption-operator.md) | **Refined 2026-09-16**, not scheduled | A Kubernetes operator provisioning proxy instances through Custom Resources. A separate program with its own entry point and its own chart; `s3-encryption-proxy` stays the single-instance one ([ADR 0033](../adr/0033-a-proxy-instance-holds-its-uploads.md)). Independent of [036](036-high-availability.md). The refining round of 2026-09-16 decided the scope — one cluster-scoped operator Deployment over namespaced Custom Resources, no cross-namespace reference inside a resource, backend credentials read from a Secret and the proxy's own client credential minted back into one, cert-manager adopted into the kind end-to-end setup — and read the security decisions of a sibling operator against this tree: twenty-five findings, lettered K-AI, on the privilege footprint a cluster-scoped controller needs, what `escalate` and a `create deployments` grant each buy an attacker, where validation can and cannot live, and what the chart already refuses at render that a resource would have to reproduce. A second round the same day answered three of them — the operator's ServiceAccount holds cluster-wide read and write on Secrets bounded on the write path by a `ValidatingAdmissionPolicy`, the Custom Resource carries a fully typed schema over the proxy's configuration keys, and the operator holds one licence token and copies it per namespace — each taken against the recommendation, with what each costs recorded as an accepted residual risk rather than as an open worry. Two ADRs are owed for the decisions; 0037 is taken, so they start at 0038. Forty questions, four closed and thirty-six open; configuration is still read at start only, so a changed backend rolls the pod | — |
| [039](039-backend-certificate-verification-failure-is-named.md) | Open (2026-09-14), **refined 2026-09-18** | A backend under a private CA: the entry gains a `ca_file` that replaces the system roots for that backend rather than adding to them, with a startup refusal for a file that cannot serve as a trust root and for a contradiction with `insecure_skip_verify` ([ADR 0037](../adr/0037-the-backend-leg-is-trusted-explicitly-and-its-failures-are-named.md)); the CA bundle in the image documented in `docs/developer/configuration.md`; and the `tls` failure class split so a certificate that does not verify is distinguishable on the counter an alert reads, at error level. The refining round took twelve decisions and found the log half **already mostly built** — the backend observer landed 2026-09-15, one day after the survey this ticket rested on, with five failure classes, a line per failed round trip and the counter pair — which deleted five of the original nine items. Retry behaviour and the backend-before-first-request question were split into [042](042-a-certificate-failure-is-not-retried.md) and [043](043-the-backend-is-checked-before-the-first-client-request.md). Two open questions, neither blocking | — |
...
| [042](042-a-certificate-failure-is-not-retried.md) | Split out 2026-09-18, **decision only** | A backend certificate that cannot be verified is retried to the SDK's attempt ceiling — three TLS handshakes per client request — because `*smithyhttp.RequestSendError` reports `ConnectionError()` as true unconditionally, and no retry can succeed until a person acts. Verified in the module cache. Whether to stop the retry, and whether the certificate case is the only one that stops, is open | — |
| [043](043-the-backend-is-checked-before-the-first-client-request.md) | Split out 2026-09-18, **decision only** | Two attributions of a backend failure to the wrong thing: a backend the proxy cannot use produces a pod that goes Ready and fails every request, and a backend fault mid-response is logged as a client disconnect. Mostly answered already — [ADR 0034](../adr/0034-a-probe-reports-the-process-never-its-dependencies.md) D6 rules out a readiness endpoint that talks to the backend and D9 refuses a startup probe *while the startup path does no network I/O*, naming the condition that reopens it. What is open is whether the start itself should touch the backend, which would need D9 amended | — |
...
refined but not scheduled: round 3 of 2026-09-16 closed twenty-three
questions — including the two that round 2 produced by withdrawing a decision
of round 1 — and leaves nine, starting at 37.

[039](039-backend-certificate-verification-failure-is-named.md) is independent of
all three, and so are the two tickets split out of it on 2026-09-18,
[042](042-a-certificate-failure-is-not-retried.md) and
[043](043-the-backend-is-checked-before-the-first-client-request.md). None of the
three is breaking; 039's new configuration key is additive inside an existing entry.
...
to schedule it, and [037](037-multiple-backends.md) because it is an announcement
— written so that 5.0.0's configuration does not foreclose it, with nothing
built. [038](038-s3-encryption-operator.md) left that group on 2026-09-16: its
scope and credential model are decided, thirty-six of its forty questions are
...
[docs/security/](../security/), in the closing section of the page whose mechanism has the gap.
