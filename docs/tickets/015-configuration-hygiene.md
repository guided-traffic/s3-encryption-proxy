...
**Open, and half its size.** The dead-code round of 2026-09-10 closed the deletion
half: items 1, 3, 7, 8 (deletion part), 12 and 13. What is left is the half that
*adds* something — three decisions ADR 0013 records as specified-and-not-built,
one prefix-shape change, one new item for ADR 0013 D11, plus the test, doc and
...
2/4/5/6), **11** (verification), **14** (prefix shape), **15** (D11, unknown key
...
else. Every anchor below was read in the tree at `6677e4e`.
...
|---|---|---|
| **1** — delete `SecurityMetrics`, the failed-attempt map, `getClientIP`, the brute-force branch | dead-code round 2026-09-10 | `grep -rn SecurityMetrics internal/ cmd/ pkg/` is empty. `logSecurityEvent` ([s3auth_robust.go:407-417](../../internal/proxy/middleware/s3auth_robust.go#L407-L417)) logs `remote_addr` and `x_forwarded_for` as two raw fields, exactly as ADR 0013 D2 specifies. The three remaining `SecurityMetrics` hits are the Prometheus subtest name in [auth_test.go:143-144, 430](../../test/integration/authentication/auth_test.go#L143), which is what the success criterion always allowed |
| **3** — delete the six dead `s3_security` keys, `GetS3SecurityConfig`, plus `streaming_buffer_size` and `enable_adaptive_buffering` | same | `S3SecurityConfig` ([config.go:64-68](../../internal/config/config.go#L64-L68)) carries `MaxClockSkewSeconds` and nothing else; `validateS3Security` ([config.go:711-723](../../internal/config/config.go#L711-L723)) checks only that one. `OptimizationsConfig` ([config.go:70-94](../../internal/config/config.go#L70-L94)) has no buffer or threshold field. `GetStreamingBufferSize`, `GetStreamingThreshold`, `GetS3SecurityConfig`, `GetProviderByAlias`, `GetProviderConfig`, `ValidateS3ClientCredentials`, `IsS3ClientAuthEnabled` all have zero hits |
...
| **8**, deletion half — strip the dead keys from every config surface | same | Nothing left in `config/*.yaml`, `deploy/helm/…/values-production.yaml` or `test/e2e/velero/values-proxy.yaml`, the four-line D-5 comment in the latter included. Note there are **four** example configs now, not five: `config/rsa-example.yaml` went with the `rsa` provider (`2fa4b9c`). `config/multi-example.yaml` also lost a top-level `streaming.segment_size` block no code ever read |
...
Two premises the deletion round removed, so nobody re-derives them:
...
- **`TestIntegrityVerificationWithDefaults` is gone** with
  ...
  `TestLoad_ValidNoneConfig` ([config_test.go:11-40](../../internal/config/config_test.go#L11-L40)),
  which loads `s3_backend.target_endpoint: "http://localhost:9000"` with a `none`
...
- [ ] **2. ADR 0013 D3 — honour `max_clock_skew_seconds` on the header-signed path.**
      ...
      at [:232](../../internal/proxy/middleware/s3auth_robust.go#L232), while the
      ...
      comparison at [:236-239](../../internal/proxy/middleware/s3auth_robust.go#L236-L239):
...
- [ ] **4. ADR 0013 D6 / ADR 0014 D5 — `s3_security.max_presign_expiry_seconds`.**
      ...
      - `validateS3Security` ([config.go:711-723](../../internal/config/config.go#L711-L723))
      ...
      [s3auth_coverage_test.go:603](../../internal/proxy/middleware/s3auth_coverage_test.go#L603).
...
- [ ] **5. ADR 0013 D5 — refuse a plain-HTTP backend under an encrypting provider.**
      ...
      ([config.go:261-306](../../internal/config/config.go#L261-L306)) *after*
      `validateLicenseAndEncryption`, so `GetActiveProvider()`
      ([config.go:726](../../internal/config/config.go#L726)) can be trusted. When no
      provider is configured the check abstains: the configuration has other
      ...
      [main.go:76-79](../../cmd/s3-encryption-proxy/main.go#L76-L79) as a non-zero
      ...
      "failed to seek body to start". Use an https:// endpoint, or the "none"
      provider if a pass-through proxy is what you want.
      ```
      Unit tests: http × encrypting fails naming `target_endpoint`; http × `none`
      loads; https × encrypting loads; scheme-less fails; no provider + http loads.
- [ ] **6. ADR 0013 D5, the warning half.** Rewrite the hand-rolled provider loop at
      [main.go:136-147](../../cmd/s3-encryption-proxy/main.go#L136-L147) around one
      `cfg.GetActiveProvider()` call and emit both warnings from it — the existing
      "encryption is disabled" one and the plain-HTTP one. It belongs in `main`
      because it needs the logger, configured immediately above at
      ...
      ```
      "may still fail" is not hedging — see the probe below, and note the code
      reading has got *stronger* since this was written: `putObjectSegmented` hands
      `h.requestParser.StreamingReader(r)` to `PutObject` for the pass-through
      provider as well ([operations.go:242-256](../../internal/proxy/handlers/object/operations.go#L242-L256)),
      and the multipart producer's part bodies are sealing readers over a pooled
      buffer, not `bytes.Reader`s ([segmented.go:142-144](../../internal/orchestration/segmented.go#L142-L144)).
      Nothing on either write path is seekable. If the probe confirms it, this
      warning becomes a second refusal — a one-line change.
...
- [ ] **10. Docs for items 2, 4, 5, 6 and 14.** The dead-key documentation is
      ...
      "No rate limiting" bullet ([README.md:895](../../README.md#L895)) cites
      ADR 0014, and `SECURITY_ARCHITECTURE.md` H-7 is closed. What the open items
      still owe:
      - [README.md:286-288](../../README.md#L286-L288): the `s3_security` block —
        ...
        with item 4;
      - [README.md:740-748](../../README.md#L740-L748), "Pre-signed URLs": it still
      ...
      - the `s3_backend` section: one line saying an `https://` `target_endpoint` is
        required unless the provider is `none`, and that a scheme-less endpoint is
        refused (items 5 and 6);
      - item 2 breaks a client whose clock is between 300 s and 900 s off. Say so
        ...
        deployment;
      - `SECURITY_ARCHITECTURE.md` §6.3 and the H-10 checklist
        ([:825-848](../../SECURITY_ARCHITECTURE.md#L825-L848)): tick what lands, and
        close H-10 when all three do;
      - mirror both config blocks into `CLAUDE.md`.
      [023](023-major-v5.md) already carries the release-notes lines for items 2, 4,
      5, 14 and 15 under "Configuration — refuses to start"; check them against what
      actually ships rather than writing new ones.
...
- [ ] **15. ADR 0013 D11 — an unknown configuration key refuses the start, and the
      ...
      - **The loader.** `Load()` calls `viper.Unmarshal(&cfg)` with no options
        ([config.go:175-198](../../internal/config/config.go#L175-L198)); it is the
        only `viper.Unmarshal` call in the tree. Pass a
        `viper.DecoderConfigOption` — `func(*mapstructure.DecoderConfig)`, viper
        1.21.0 `viper.go:92` and `:935` — that sets `ErrorUnused: true`. The
        `mapstructure` in that signature is `github.com/go-viper/mapstructure/v2`,
        today an indirect dependency ([go.mod:39](../../go.mod#L39)); importing it
        promotes it to a direct one, so `go mod tidy` runs in the same change.
      ...
      - **The catch-all must keep working.** `EncryptionProvider.Config` is
        ...
        (`mapstructure.go:1596-1599`), so a provider block keeps swallowing its own
        parameters. That is ADR 0013 D11's first boundary — assert it with a test,
        do not assume it. `loadProviderConfigs` ([config.go:360-381](../../internal/config/config.go#L360-L381))
      ...
      - **Environment variables are out of scope** (D11's second boundary).
        `viper.AutomaticEnv()` with prefix `S3EP` ([config.go:161-163](../../internal/config/config.go#L161-L163))
        contributes nothing to `AllSettings`, so no work is needed — confirm rather
        than assume.
      - **The four shipped examples must pass.** Three already did when this was
        measured; `config/multi-example.yaml` was the fourth and its dead
        `streaming.segment_size` block is already gone. Re-run the check against all
        four after the switch is on.
      - **The rendered chart config must pass too**, and this is where the work is.
        ...
        - [values-monitoring.yaml:81-106](../../deploy/helm/s3-encryption-proxy/values-monitoring.yaml#L81-L106)
          ...
          [values-development.yaml:51-69](../../deploy/helm/s3-encryption-proxy/values-development.yaml#L51-L69)
...
- `internal/config`: `http://` + an encrypting provider fails with a message naming
  `target_endpoint`; `http://` + `none` loads; `https://` + encrypting loads; a
...
**One manual probe** (item 6, `none` provider). Start the proxy with the `none`
provider against `http://minio:9000` and PUT an object on each write path — one
below `optimizations.streaming_segment_size` (12 MiB # default), one above it, since
those are the two paths [operations.go:222-234](../../internal/proxy/handlers/object/operations.go#L222-L234)
chooses between. `streaming_threshold`, which the original probe named, no longer
exists. Record the result here. If either fails with `failed to seek body to start`,
item 6's warning becomes a refusal and ADR 0013 D5 loses its exception; record that
in the ADR rather than here.

---

## Open questions

- **The `none`-provider exception may be worthless.** D5 warns instead of refusing
  for `none` on the assumption that a pass-through body is seekable. Code reading
  says it is not, on either write path (see item 6). The probe settles it. It does
  not block the ticket: the refusal for encrypting providers is the
  security-relevant half and is unaffected.
- **`X-Forwarded-For` stays in the logs** ([s3auth_robust.go:407-417](../../internal/proxy/middleware/s3auth_robust.go#L407-L417)).
  Accepted: it is a log field, not a key or a decision input. `SECURITY_ARCHITECTURE.md`
  §4.2 ([:400](../../SECURITY_ARCHITECTURE.md#L400)) already says so and should keep
