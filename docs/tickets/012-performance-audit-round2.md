...
| 2.1 stream the client-driven `UploadPart` | **Closed 2026-09-13** | Every branch of the verb now decides on the declared plaintext length before it reads anything. A part that covers whole segments is sealed as the backend pulls it; a short last part is read under the process-wide short-part budget, because it has to be held until Complete; and the exit provider's pass-through part, the last one that was read whole without any bound, is forwarded while it arrives, with an undeclared length read under the same budget and refused above it |
