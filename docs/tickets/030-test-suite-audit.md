...
**The first five rows land with this ticket.** They are the ones where the cost is
a few lines and the consequence is a gate that stops lying. Everything below them
is a work list with no date on it.
...
**Gates after the five rows:** `go build`, `go vet`, `gofmt`, `make test-unit`,
`make test-unit-race` (0 races), `make lint` (0 issues). The integration, TLS,
conformance and Velero suites have **not** been re-run — rows 1 and 2 touch paths
those suites exercise, so they are owed before the merge.
...
- [ ] Split the predicate: `ClaimsSegmentedFormat(metadata)` next to
      `IsSegmentedObject`, true when the algorithm is the format id whatever the
      state of the wrapped key
- [ ] At each of the three exit gates, refuse with `ErrKeyMaterialUnreadable` —
...
- [ ] One test per read path, each asserting `403 InvalidObjectState` and that no
      stored byte reaches the body, table-driven over a deleted and an unparseable
      `s3ep-encrypted-dek`
- [ ] `docs/developer/storage-format.md` classes an object "carrying no wrapped
...
- [ ] Inject `S3EP_LICENSE_TOKEN` into the `unit-tests` job, the way the
      integration and conformance jobs already do
- [ ] Re-run the mutation afterwards: with the licence present the test must fail
...
- [ ] A `test-unit-race` target, and the same for the integration tag
- [ ] A CI job running it. Separate from `unit-tests` because the detector costs
      roughly 2-20× runtime and 5-10× memory
- [ ] Whatever it finds becomes rows here
...
- [ ] A vector file of constants captured once from the current build and never
...
- [ ] Assert `AADForTest` against captured AAD bytes — the cheapest pin of the
      field order, and it needs no ciphertext at all
- [ ] A fresh test-only DEK literal, not one of the KEK fixtures already in the
      tree, so it can never be copied out of a test into a configuration (ADR 0021)
- [ ] A header comment saying these constants describe the stored format, are not
...
- [ ] Fail each readiness step when its loop is exhausted
...
- [ ] **ADR 0011 D5 is not built.** D5: *"Across all sessions, buffered short-part
...
- [ ] **A large PUT cut short by the shutdown budget orphans a backend multipart
...
- [ ] **A licence that lapses at runtime calls `os.Exit(1)`** from the monitoring
...
- [ ] **The two end-relative ranged reads take a HEAD and then a GET with nothing
...
- [ ] **The prefix body of a tail-first GET is closed by a `defer` whose guard no
...
- [ ] **The range tail test's negative case dies in `openSegment`**, so the
...
- [ ] **`TestChkTrailerBlockIsBounded` cannot fail**: its 50,000 junk lines are
...
- [ ] **`perf-compare` judges absolute medians** instead of re-forming the
...
- [ ] **`TestServerTLSConfiguration` skips both subtests on every run.** It reads
...
- [ ] **`"no plaintext byte is written before the refusal"` is the tautology
      `0 == 0`**
- [ ] **The chart's `checksum/config` assertion compares against a stale literal**,
      so it is dead. The bug it was written for is caught by the Velero e2e, which
      is why nobody noticed
- [ ] **`TestV8b`'s descriptor scan can match nothing and still pass** (ADR 0019 D12)
- [ ] **The conformance refusal cost guard is vacuous**: the cost file sorts before
      the refusal file, so it lists a prefix nothing has written to yet. All three
      cost guards audit the *previous* run **(not probed)**
- [ ] **`TestOptimizationsConfig` never compares the error message it declares**
- [ ] **`TestRtPxMetadataPrefixResolution` pins a getter whose only consumer
      discards it**, and its empty-prefix case contradicts ADR 0009 D2
- [ ] **The namespace assertion in the `BuildSegmentedMetadata` collision test
      cannot fail**, and its comment claims a guard the function does not have
- [ ] **Seven subtests in the integration auth suite end in `t.Logf`**, two of them
      named after security boundaries
- [ ] **The dead throughput gate** ADR 0020 D11 forbids: disarmed on every path
      that runs it. Deleting it loses no coverage
- [ ] **`TestMultipartUploadCorruption` asserts nothing about size or hash**; its
      coverage is already given by the 1 GB comprehensive test

## Invariants nothing pins

- [ ] **ADR 0002 D1 — "a data key is never reused across objects".** Hoist the
      ...
      compares the keys
- [ ] **ADR 0004 D7 — the wrap derivation labels `s3ep-kek-wrap-v1` and
...
- [ ] **ADR 0028 D1 — expiry measured from the last part received.** The idle-clock
      ...
      at 3600 s
- [ ] **The aligned last part of a client-driven upload.** All three orders in the
...
- [ ] **`multipart_upload_concurrency` has no behavioural test**: pin the producer
      to one worker and every package stays green
- [ ] **ADR 0003 D5 — the bucket is absent from the associated data** — is asserted
      by no test
- [ ] **A completed upload freeing its session** is unasserted; only the abort path
      is pinned
- [ ] **A real `AbortMultipartUpload` driven from the sweep** is never observed at
      any layer; the abandoner closure is unexercised
- [ ] **The four listener budgets reaching the data-plane server** are unasserted,
...
- [ ] **The shutdown deadline's anchor**: no test pins it to the shutdown start,
      and the anchor is written twice by hand
- [ ] **ADR 0025 D10 and ADR 0013 D5 — the two startup warnings** (an active exit
...
- [ ] **ADR 0013 D8 — `pprof_enabled` stands on its own.** The rule is one `if`
      outside the monitoring block, in untested wiring; re-nesting it restores the
      defect the comment there records
- [ ] **A configured `max_presign_expiry_seconds`** is never driven through
...
- [ ] **Route-to-auth binding is pinned for 6 of 31 routes.** A route on the health
      router, or a new root-router route the catch-alls do not shadow, serves
      unsigned with no test failing
- [ ] **A GET/HEAD of a zero-byte backend object** is never tested; the
      416 → `ErrForeignObject` mapping is unexercised
- [ ] **Neither `ErrCorrupt` guard in the tail fetch is reached**, and the case
      named for the length guard exits three lines earlier
- [ ] **Every ranged-read fault injection hits the first segment of the window**; a
      fault in a later segment is untested
- [ ] **An unimplemented algorithm named in `X-Amz-Trailer`** is never tested; only
      the request-header form is
- [ ] **The 5 MB floor on `multipart_short_part_buffer_size`** has no test
- [ ] **`config/multi-example.yaml` is the one shipped config nothing loads**, so
      decode is its entire coverage
- [ ] **`cmd/keygen` has no test file.** It produces the KEK an operator pastes
...
- [ ] **The four untagged `bucket_{acl,cors,location,logging}_test.go` files are
...
- [ ] **The 39-cell sub-resource matrix runs against a hand-copied router** with no
...
- [ ] **The batch-delete shape test pins the missing S3 namespace on
      `<DeleteResult>` as expected behaviour**, against ADR 0008 D3 and the file's
      own oracle contract **(not probed)**
- [ ] **`TestSegmentedTrailerAnswersHead` never opens the trailer** it is named for,
      and its comment describes the pre-D14 HEAD
- [ ] **"A refused upload must leave no object behind" is asserted as any error**,
      so a 403 from a committed truncated object reads as success
- [ ] **The owner-guard source walk checks the field is present**, not that it
      carries the client's value
- [ ] **The S3 namespace is asserted on 5 of 12 bucket sub-resource documents**
- [ ] **No LIST entry's ETag is read by any end-to-end test**, so no run proves
      LIST, GET and HEAD name the same entity tag against a real backend
- [ ] **The unsatisfiable-range table holds a byte-identical duplicate** whose
      comment describes a backend-416 path the test never reaches
- [ ] **The exit-provider ranged sub-test pins a 206 that contradicts the project's
      own recorded decision**, using a scenario no real backend produces, while the
      reachable scenario is untested
- [ ] **Three comments in the out-of-order multipart test still describe the deleted
      AES-CTR ordering pipeline**, contradicting the subtests below them
- [ ] **The sealed-CRC detector test discards the released plaintext.** At an exact
...
- [ ] **A stale comment on the GET path** claims closing the reader is what makes a
      failure visible; the codec reader's `Close()` returns nil unconditionally and
      the error path skips it anyway

## Green by skipping

- [ ] **`t.Skip(err)` as error handling**: eight occurrences in the memory
...
- [ ] **MAIN GOAL 3 has no gate.** The memory test says so itself: *"It records
...
- [ ] **The throughput size set never reaches the multipart producer**, though its
      comment says it does
- [ ] **20 hex dumps of plaintext payloads** sit on the success path of the
      360-degree suites, against WORK ORDER 1. The comparison itself correctly uses
      SHA-256
- [ ] **The conformance seed-completeness check runs last** and covers 7 of 17
      seeded objects, so the precondition it exists to enforce lands after the
      cascade it was written to pre-empt
- [ ] **Nothing couples the conformance run script's segment size to the corpus
      size**, so the multipart split can stop happening silently
- [ ] **The conformance budget's ceiling refusal is untested and its zero-limit
...
- [ ] Rows 1 to 5 are on the branch and each is re-verified by re-running the
      mutation that proved it
- [ ] Every remaining row is either done, or moved to a ticket of its own with a
      reason
- [ ] The ADR contradictions this audit surfaced are settled in the ADR or in the
