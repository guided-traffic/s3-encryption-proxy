...
| [0023](0023-filename-encryption-encrypts-directory-segments.md) | Filename encryption encrypts every segment of the key deterministically, with a searchable head on the leaf and no mapping index; a mode set `off / drain / mixed / strict` serves both name forms in one bucket (amended 2026-09-17; the file keeps its original name) | Not built |
...
| [0033](0033-a-proxy-instance-holds-its-uploads.md) | A client-driven multipart upload is held by the process that created it, so the chart installs one instance and refuses a second; cooperating proxies are a separate product | Implemented |
...
| [0037](0037-the-backend-leg-is-trusted-explicitly-and-its-failures-are-named.md) | A backend entry names its own trust with `ca_file`, which replaces the system roots for that backend rather than adding to them; an unusable file or a contradiction with `insecure_skip_verify` refuses the start, and a certificate that does not verify is its own failure class at error level while the response stays `500 InternalError` | Decided, not built |
...
| [0022](0022-tickets-are-work-lists-that-get-archived.md) | Tickets are archived when the work lands; every durable decision lives in an ADR, and nothing outside the ticket directory cites a ticket | Partly built |
