...
import (
	...
	internalConfig "github.com/guided-traffic/s3-encryption-proxy/internal/config"
...
func (m *mockS3Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("Mock S3 Request: %s %s (bucket=%s, key=%s)\n", r.Method, r.URL.String(), bucket, key)
...
<InitiateMultipartUploadResult>
    ...
    <UploadId>%s</UploadId>
			...
			fmt.Printf("Mock S3 Response: Created upload %s\n", uploadID)
...
<CompleteMultipartUploadResult>
	...
	case "PUT":
		if uploadID := r.URL.Query().Get("uploadId"); uploadID != "" {
			...
			w.WriteHeader(http.StatusOK)
			fmt.Printf("Mock S3 Response: Uploaded part %d for upload %s\n", partNumber, uploadID)
		} else {
...
func setupIntegrationTest(t *testing.T) (http.Handler, *mockS3Server, string) {
	// Create mock S3 server
	mockS3 := newMockS3Server()
	s3Server := httptest.NewServer(mockS3)

	// Create config for AES-GCM encryption
	cfg := &internalConfig.Config{
		...
		Encryption: internalConfig.EncryptionConfig{
			EncryptionMethodAlias: "test-aes",
			Providers: []internalConfig.EncryptionProvider{
				{
					Alias: "test-aes",
					Type:  "aes-gcm",
					Config: map[string]interface{}{
						"aes_key":             "dGVzdC1rZXktMzItYnl0ZXMtZm9yLWFlcy0yNTYhISE=", // base64 encoded
	...
	return handler, mockS3, s3Server.URL
}

func TestMultipartUploadEncryptionIntegration(t *testing.T) {
	handler, mockS3, _ := setupIntegrationTest(t)
